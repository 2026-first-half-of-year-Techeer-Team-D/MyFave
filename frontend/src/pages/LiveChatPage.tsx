...
export function LiveChatPage() {
  ...
  return (
    <div className="relative flex flex-1 flex-col bg-white overflow-hidden min-h-0">
      {/* Background Layer: Shop Page Content (Figma Node 37:3357 명세) */}
      <div className="absolute inset-0 z-0 opacity-15 pointer-events-none overflow-hidden">
        ...
        <div className="flex gap-[24px] px-[20px] py-[16px] border-b-[1.096px] border-separator bg-white">
          <span className="font-noto text-[14px] font-medium text-dark-text border-b-2 border-dark-text pb-1">전체</span>
          <span className="font-noto text-[14px] font-medium text-muted-text">상의</span>
          <span className="font-noto text-[14px] font-medium text-muted-text">하의</span>
          <span className="font-noto text-[14px] font-medium text-muted-text">아우터</span>
          <span className="font-noto text-[14px] font-medium text-muted-text">악세사리</span>
        </div>
        <div className="mx-auto max-w-md px-[20px] grid grid-cols-2 gap-x-[15.15px] gap-y-[32px] pt-[32px]">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="flex flex-col">
              <div className="h-[200.25px] bg-gray-100 rounded-[12px] mb-[12px] border border-separator/10" />
              <div className="h-[16px] bg-gray-200 w-full mb-[4px] rounded-[2px]" />
              <div className="h-[16px] bg-point/10 w-[60%] rounded-[2px]" />
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10 flex flex-1 flex-col bg-[#FFF9F0]/85 backdrop-blur-[2px]">
        {/* 1. Header Notice Card - Figma Node 99:267 & style_8F31OE */}
        ...
        {/* 2. Participant Badge - Figma Node 99:540 & fill_721VTE */}
        ...
        {/* 3. Chat Messages Area - Figma layout_Q6ETGY 간격 (17px) */}
        <div 
          ref={scrollRef}
          className="flex-1 overflow-y-auto px-[19.99px] pb-[120px] scrollbar-hide"
        >
              ...
              <div key={msg.id} className={`flex items-start gap-[8.5px] ${msg.user === '나' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`flex flex-col gap-[4.5px] ${msg.user === '나' ? 'items-end' : ''}`}>
                  <div className="flex items-center gap-[6px] px-[2px]">
                    ...
                    {msg.isOfficial && (
                      <span className="rounded-[2px] bg-point px-[4px] py-[1px] text-[8px] font-black text-white tracking-tighter uppercase">My Fave 공식</span>
                    )}
                  </div>
                    ...
                    <div 
                      className={`rounded-[15.5px] px-[16px] py-[9.5px] shadow-sm border border-separator/5 ${
                        msg.user === '나' 
                          ...
                          : msg.isOfficial
                            ? 'rounded-tl-none bg-main-bg text-dark-text font-medium'
                            : 'rounded-tl-none bg-[#FFF7F8] text-dark-text font-medium'
                      }`}
                    >
                      <p className="font-noto text-[12px] font-normal leading-[18.2px] tracking-tight">
                        {msg.text}
                      </p>
                    </div>
      ...
      {/* 4. Bottom Sticky Input Bar - Figma Rectangle 8 & 9 명세 */}
        ...
        <form onSubmit={handleSend} className="flex items-center gap-[12px]">
          <div className="relative flex-1">
            <input
              ...
              className="w-full rounded-[21px] border border-separator bg-white px-5 py-[11px] font-noto text-[12px] text-dark-text placeholder:text-muted-text/40 focus:border-point focus:outline-none transition-colors shadow-inner"
            />
          </div>
          <button
            type="submit"
