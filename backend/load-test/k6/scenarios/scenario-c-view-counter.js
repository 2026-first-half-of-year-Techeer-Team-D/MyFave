// 시나리오 C — 조회수 카운터 집중 (A-2 측정용)
// 인기 콘텐츠 1개에 조회(POST view)를 몰아넣어 카운터 쓰기 경합을 유발.
// 목적: counter-mode=db(조회마다 DB 직접 +1, 행 락 경합) vs redis(INCR, 락 없음) 대비.
// 운영법: 백엔드 content.view.counter-mode 를 db / redis 로 바꿔가며 2회 측정 후 비교.
...
// 경합을 보려고 단일 인기 콘텐츠에 집중 (모든 VU가 같은 row/key를 때림)
const HOT_TYPE = __ENV.HOT_TYPE || 'style_feed'; // short_form | style_feed
const HOT_ID = __ENV.HOT_ID || '1';
...
export function setup() {
  console.log(`[scenario-C] BASE_URL=${BASE_URL}, 핫콘텐츠=${HOT_TYPE}/${HOT_ID}, 토큰 풀=${tokens.length}`);
}
...
export default function () {
  ...
  // 단일 인기 콘텐츠에 조회수 몰빵 — 카운터 쓰기 경합 집중
  const res = post(`/content/${HOT_TYPE}/${HOT_ID}/view`, accessToken, {});
